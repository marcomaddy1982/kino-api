...
class V1::Auth::SessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "login returns 200 with tokens and user" do
    post v1_auth_login_path, params: { email: "existing@example.com", password: "password" }, as: :json
  ...
  test "login returns 401 on unknown email" do
    post v1_auth_login_path, params: { email: "nobody@example.com", password: "password" }, as: :json
    assert_response :unauthorized
  end

  # POST /v1/auth/refresh
  test "refresh returns 200 with new tokens" do
    result = AuthService.login(email: "existing@example.com", password: "password")
